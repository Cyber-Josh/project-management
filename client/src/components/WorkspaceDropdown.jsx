        ...
        <div className="relative m-4" ref={dropdownRef}>
            <button onClick={() => setIsOpen((prev) => !prev)} className="w-full flex items-center justify-between p-3 h-auto text-left rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                <div className="flex items-center gap-3">
                    <img src={currentWorkspace?.image_url || assets.workspace_img_default} alt={currentWorkspace?.name || "Workspace"} className="w-8 h-8 rounded shadow" />
            ...
            {isOpen && (
                <div className="absolute z-50 w-64 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded shadow-lg top-full left-0">
                    <div className="p-2">
                        ...
                        {userMemberships.data.map(({organization}) => (
                            <div key={organization.id} onClick={() => onSelectWorkspace(organization.id)} className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                                <img src={organization?.imageUrl || assets.workspace_img_default} alt={organization?.name || 'Workspace'} className="w-6 h-6 rounded" />
                    ...
                    <hr className="border-gray-200 dark:border-zinc-700" />
                     ...
                     <div onClick={() => { setIsOpen(false); setIsCreateOrgOpen(true); }} className="w-full text-left p-2 cursor-pointer rounded group hover:bg-gray-100 dark:hover:bg-zinc-800" >
                ...
                <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="relative">
                        <button
                            ...
                            className="absolute -top-3 -right-3 size-8 rounded-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300"
                            aria-label="Close create workspace dialog"
                        >
                            x
                        </button>
                        <CreateOrganization afterCreateOrganizationUrl="/" afterSelectOrganizationUrl="/" />
                    </div>
                </div>
            )}
        </div>
